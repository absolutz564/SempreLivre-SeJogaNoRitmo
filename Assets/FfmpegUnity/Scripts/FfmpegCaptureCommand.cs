...
namespace FfmpegUnity
{
    ...
    {
        const int CAPTURE_VIDEO_LIMIT_PER_FRAME = 3;
        ...
        string[] initCoroutineCommands_ = null;

        bool writeThreadIsError_ = false;

        List<FfmpegCaptureCamera> captureCameras_ = new List<FfmpegCaptureCamera>();

        public void Stop()
        {
            UnityEngine.Debug.Log("chamou stop ffmpeg");
            StopFfmpeg();
        }
        protected override void Build()
        {
            //if (activeCaptureCoroutine != null)
            //{
            //    StopCoroutine(activeCaptureCoroutine);
            //    UnityEngine.Debug.Log("Coroutine anterior interrompida.");
            //}

            activeCaptureCoroutine = StartCoroutine(captureCoroutine());
        }
        ...
        protected override void Clean()
        {
            ...
            */
            ...
            foreach (var captureCamera in captureCameras_)
            {
                Destroy(captureCamera);
            }
            captureCameras_.Clear();
        }
        ...
        IEnumerator captureInitCoroutine(int captureLoop)
        {
                ...
                case CaptureSource.SourceType.Video_GameView:
                    {
                        ...
                        initCoroutineCommands_[captureLoop] = " -r " + CaptureSources[captureLoop].FrameRate + " -f rawvideo -s " + width + "x" + height + " -pix_fmt " + textureFormat + " -i \"" + fileName + "\" ";
                    }
                    break;
                case CaptureSource.SourceType.Video_Camera:
                    {
                        ...
                        captureCameras_.Add(captureCamera);
                ...
                case CaptureSource.SourceType.Audio_AudioListener:
                    {
                        ...
                        initCoroutineCommands_[captureLoop] = " -f f32le -ar " + AudioSettings.outputSampleRate + " -ac " + audioChannels_[captureLoop] + " -i \"" + fileName + "\" ";
                    }
                    break;
                case CaptureSource.SourceType.Audio_AudioSource:
                    {
                        CaptureSources[captureLoop].SourceAudio.Play();
        ...
        private Coroutine activeCaptureCoroutine;
        public void ForceStopAndClean()
        {
            UnityEngine.Debug.Log("Forçando parada total do FFmpeg...");
        ...
        {
            UnityEngine.Debug.Log("Começou record...");
...
//            captureImp_ = new FfmpegCaptureImpAndroidMemory(this);
            ...
            initCoroutineCommands_ = new string[CaptureSources.Length];
            for (int captureLoop = 0; captureLoop < CaptureSources.Length; captureLoop++)
            {
                StartCoroutine(captureInitCoroutine(captureLoop));
            }
            ...
            {
                breakFlag = true;
                foreach (var coroutineCommand in initCoroutineCommands_)
                {
                    if (string.IsNullOrEmpty(coroutineCommand))
                    {
                        breakFlag = false;
                        break;
                    }
            ...
            {
                yield return null;
                if (writeThreadIsError_)
                {
                    CommandImp.IsForceStop = true;
                    CommandImp.StopFfmpeg();
                    writeThreadIsError_ = false;
                    CommandImp.IsForceStop = false;
                    StopFfmpeg();
                    yield break;
                }
            } while (captureImp_.IsStandby);
                    ...
                    if (CaptureSources[captureLoop].Type <= CaptureSource.SourceType.Video_RenderTexture)
                    {
                        ...
                        if (SystemInfo.supportsAsyncGPUReadback)
                        {
                            if (CaptureSources[captureLoop].Type != CaptureSource.SourceType.Video_RenderTexture || reverse_[captureLoop])
                            {
                                ...
                                {
                                    Destroy(renderTextures_[captureLoop]);
                        ...
                        if (nextCaptureTime_[captureLoop] < Time.realtimeSinceStartup)
                        {
                            nextCaptureTime_[captureLoop] = Time.realtimeSinceStartup;
                        }
                    }
            ...
            tempTextures_.Clear();
            UnityEngine.Debug.Log("Terminou record...");

           // Debug.Log("Chama no final pra enviar ao servidor");
           // string videoFilePath = $"{Application.persistentDataPath}/capture.mp4";

           // // Envia o vídeo após FFmpeg finalizar
           // while (!File.Exists(videoFilePath))
           // {
           //     Debug.LogWarning("Arquivo ainda não disponível, aguardando...");
           //     yield return new WaitForSeconds(0.5f);
           // }
           //backButton.UploadVideo(videoFilePath);
        }
        ...
        public void CaptureCameraRenderImage(RenderTexture srcTexture, int captureId)
        {
            ...
            if (CaptureSources[captureId].Type != CaptureSource.SourceType.Video_RenderTexture || reverse_[captureId])
            {
                ...
                {
                    Destroy(renderTextures_[captureId]);
