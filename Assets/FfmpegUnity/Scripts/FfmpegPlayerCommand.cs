...
namespace FfmpegUnity
{
    public class FfmpegPlayerCommand : FfmpegCommand
    {
        ...
        protected virtual bool IntPtrMode
        {
            get
            {
                return false;
        ...
        Dictionary<int, byte[]> resultVideoBuffers_ = new Dictionary<int, byte[]>();
        protected byte[] GetResultVideoBuffer(int id, out int width, out int height)
        {
            width = widths_[id];
            height = heights_[id];
            if (resultVideoBuffers_.ContainsKey(id))
            {
                return resultVideoBuffers_[id];
            }
            return null;
        }
        public int VideoBuffersCount = 1;
        ...
        protected IEnumerator ResetCoroutine(bool maintainPlayerImp)
        {
            ...
            if (IntPtrMode)
            {
                VideoTextures = new FfmpegPlayerVideoTexture[VideoBuffersCount];
            }
            ...
            foreach (var ffmpegStream in Streams)
            {
                if (ffmpegStream.CodecType == FfmpegStream.Type.VIDEO)
                {
                    ...
                    else if (videoTexture.VideoTexture == null)
                    {
        ...
        protected bool WriteNextTexture()
        {
            ...
            lock (videoBuffers_)
            {
                ...
                foreach (var videoBuffer in videoBuffers_)
                {
                    ...
                    {
                        resultVideoBuffers_[videoBuffer.Key] = videoBuffer.Value[0];
                    }
